...
def train_one_epoch(sess, ops, train_writer):
    ...
    # Shuffle train files
    train_file_idxs = np.arange(0, len(TRAIN_FILES)) # len(TRAIN_FILES)=5
    np.random.shuffle(train_file_idxs)
    
    for fn in range(len(TRAIN_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label = provider.loadDataFile(os.path.join('../',TRAIN_FILES[train_file_idxs[fn]]))
        current_data = current_data[:,0:NUM_POINT,:]
        current_data, current_label= provider.shuffle_data(current_data, np.squeeze(current_label))            
        current_label = np.squeeze(current_label)
        
        file_size = current_data.shape[0] # 2048
        num_batches = file_size // BATCH_SIZE # 64
        ...
        for batch_idx in range(num_batches):
            ...
            # rotation
            rotated_data = provider.rotate_point_cloud(current_data[start_idx:end_idx, :, 0:3])  # z rotation
            #rotated_data = provider.rotate_point_cloud_so3(current_data[start_idx:end_idx, :, :]) # so3 rotation

            feed_dict = {ops['pointclouds_pl']: rotated_data,
                         ops['labels_pl']: current_label[start_idx:end_idx],
                         ops['is_training_pl']: is_training,}
            summary, step, _, loss_val, pred_val, features = sess.run([ops['merged'], ops['step'],
                ops['train_op'], ops['loss'], ops['prediction_op'], ops['features']], feed_dict=feed_dict)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            rotated_data = provider.rotate_point_cloud_so3(current_data[start_idx:end_idx, :, 0:3]) # so3 rotation
            feed_dict = {ops['pointclouds_pl']: rotated_data,
                         ops['labels_pl']: current_label[start_idx:end_idx],
                         ops['is_training_pl']: is_training}
